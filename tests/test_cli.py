...
def test_escala_cli_deve_retornar_erro_ao_stdout():
...
def test_escala_cli_deve_conter_as_notas_na_resposta_em_fa(nota):
    result = runner.invoke(app, ['escala', 'F'])
    assert nota in result.stdout


@mark.parametrize('grau', ['I', 'II', 'III', 'IV', 'V', 'VI', 'VII'])
def test_escala_cli_deve_conter_as_notas_na_resposta_em_fa(grau):
    result = runner.invoke(app, ['escala', 'B'])
...
def test_acorde_cli_deve_conter_as_notas_na_resposta_em_fa(grau):
    result = runner.invoke(app, ['escala', 'B'])
    assert grau in result.stdout
